...
router.post("/import/product", async (req, res): Promise<void> => {
  ...
  try {
    ...
    // Validate redirect target isn't internal (best-effort — fetch already followed)
    try {
      const redirectedUrl = new URL(finalUrl);
      await assertSafeHost(redirectedUrl.hostname);
    } catch {
      req.log.warn({ finalUrl }, "SSRF guard blocked redirect destination");
      res.status(400).json({ error: "The URL redirected to an internal address." });
      return;
    }
